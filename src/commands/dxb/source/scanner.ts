...
type DXBScannerResult = {
  result: boolean;
}
Messages.importMessagesDirectory(__dirname);
const messages = Messages.loadMessages('dxb', 'source.scanner');
export default class DXBScanner extends SfCommand<DXBScannerResult> {
  ...
  public static readonly flags = {
    ...
    'high-severity-rules': Flags.string({ char: 'r', summary: messages.getMessage('flags.high-severity-rules.summary'), multiple: true})
  };

  public static readonly requiresProject = true;

  public async run(): Promise<DXBScannerResult> {
    const {flags} = await this.parse(DXBScanner);
    ...
    results.forEach((elem: {fileName: string; violations: Violation[]}) => {
      let content = '';
      const fileJson: any = path.parse(elem.fileName);
      if (elem.violations && !excludedFiles.includes(fileJson.name)){
        elem.violations.forEach( v => {
          if (parseInt(v.severity, 10) <= severity || ((highseverityrules?.includes(v.ruleName)) ?? (config?.dxb?.highseverityrules?.highseverityrules.includes(v.ruleName)))) {
