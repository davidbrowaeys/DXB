...
export default class ProfileConvert extends SfCommand<ProfileConvertResult> {
  ...
  public static readonly flags = {
    ...
    'source-dir': Flags.directory({ char: 'r', summary: messages.getMessage('flags.source-dir.summary'), default: 'force-app/main/default/profiles', exists: true })
  };
  ...
  public async convertProfile(profilename: string, sourcepath: string): Promise<void> {
    ...
    try {
      ...
      if (result.Profile.applicationVisibilities) {
        this.splitProfile(`${profilepath}/applicationVisibilities`, result.Profile.applicationVisibilities, () => 'application');
      }
      // classAccesses
      ...
      if (result.Profile.categoryGroupVisibilities) {
        this.splitProfile(`${profilepath}/categoryGroupVisibilities`, result.Profile.categoryGroupVisibilities, () => 'dataCategoryGroup');
      }
      // customMetadataTypeAccesses
      if (result.Profile.customMetadataTypeAccesses) {
        this.splitProfile(`${profilepath}/customMetadataTypeAccesses`, result.Profile.customMetadataTypeAccesses, () => 'name');
      }
      // objectPermissions
      if (result.Profile.objectPermissions) {
        ...
        result.Profile.objectPermissions.forEach((elem: any) => {
      ...
      if (result.Profile.fieldPermissions) {
        this.splitObjectPermissions(`${profilepath}/objectPermissions`, result.Profile.fieldPermissions, 'fieldPermissions', () => 'field');
      }
      // recordTypeVisibilities
      if (result.Profile.recordTypeVisibilities) {
        this.splitObjectPermissions(`${profilepath}/objectPermissions`, result.Profile.recordTypeVisibilities, 'recordTypeVisibilities', () => 'recordType');
      }
      // customPermissions
      ...
      if (result.Profile.externalDataSourceAccesses) {
        this.splitProfile(`${profilepath}/externalDataSourceAccesses`, result.Profile.externalDataSourceAccesses, () => 'externalDataSource');
      }
      // flowAccesses
      ...
      if (result.Profile.layoutAssignments) {
        this.splitProfile(
          ...
          (elem) => elem.recordType ? `${elem.layout.split('-')[0]}-${elem.recordType}` : `${elem.layout}`
          );
      }
  ...
  // eslint-disable-next-line class-methods-use-this
  private splitObjectPermissions(objectPath: string, profileSetting: any, permissionName: string, determineKey: (e?: any) => string): void {
    ...
    profileSetting.forEach((elem: any) => {
      ...
      fs.writeFileSync(`${objectPath}/${objectName}/${permissionName}/${elem[key]}.json`, JSON.stringify(elem, null, 2));
    });
  }
}