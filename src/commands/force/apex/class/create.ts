...
export default class ApexClassCreate extends SfdxCommand {
  ...
  `$ deloitte force:apex:class:create -n MySuperBatch -t BatchApexClass
  ...
  protected static flagsConfig = {
    ...
    defaultapiversion : flags.string({
        char: 'v',
  ...
  public async run() {
      ...
      if (!config.plugins || !config.plugins.deloitteforce){
        throw new SfdxError('Plugin definition deloitteforce is missed in sfdx-project.json, make sure to setup plugin.');
      }

      let apiname = this.flags.apiname;
      let apiversion = this.flags.apiversion ? this.flags.defaultapiversion : config.sourceApiVersion;
