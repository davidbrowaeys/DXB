...
export default class DeltaGenerate extends SfdxCommand {
  ...
  public static description = 'command to copy metadata dependencies for delta deployment';

  public static examples = [
  `$ deloitte force:delta:generate -r delta -m tags
  <list of all files changed from latest commit to head>
  `,
  `$ deloitte force:delta:generate -r delta -m commitid -c 123456
  <list of all files changed for a specific commit>
  `,
  `$ deloitte force:delta:generate -r delta -m branch -b origin/master
  <list of all files changed from latest commit to head>
  `
  ];
  ...
  public async run() {
    ...
    for(var i in files){
      ...
      var basedir = 'force-app/main/default'; //store base folder (force-app) into deloitteforce cli config json file so we can easily change it 
