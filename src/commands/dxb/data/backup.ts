...
export default class DataBackup extends SfdxCommand {
    ...
    public static examples = [
        `sfdx dxb:data:backup -m full -d backup -f config/backup-def.json -u devhub`,
        `sfdx dxb:data:backup -m delta -d backup -f config/backup-def.json -u devhub`
    ];
    ...
    public async run() {
        ...
        if (mode === 'full'){   //run full mode using bulk api
            ...
            config.full.objects.forEach(object => {
                this.ux.log(`\nBackup ${object} :`);
                exec(`sfdx dxb:data:bulk:query -o ${object} -u ${orgname} --allfields -d ${outputdir}`);
            ...
            config.delta.objects.forEach( async (object) => {
                ...
                await this.queryAll(this.org.getConnection(), soql1, filename);
