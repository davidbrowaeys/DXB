...
type PasswordPoliciesMergeResult = {
  success: boolean;
}
...
export default class PasswordPoliciesMerge extends SfCommand<PasswordPoliciesMergeResult> {
  ...
  public async run(): Promise<PasswordPoliciesMergeResult> {
    const {flags} = await this.parse(PasswordPoliciesMerge);
  ...
  public getAllPasswordPolicies(): Array<{ filePath: string; fullName: string }> {
    this.log(`sf project retrieve start --metadata ProfilePasswordPolicy --target-org ${this.orgName} --json --output-dir ${TARGET_MAIN_DIR}`);
    const metadata: { result: { inboundFiles: Array<{filePath: string; fullName: string}>}}
    = JSON.parse(exec(`sf project retrieve start --metadata ProfilePasswordPolicy --target-org ${this.orgName} --json --output-dir ${TARGET_MAIN_DIR}`).toString());
