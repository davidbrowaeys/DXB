...
export default class Delta extends SfCommand<DeltaResult> {
  ...
  public static readonly flags = {
    ...
    mode: Flags.string({ char: 'm', summary: messages.getMessage('flags.mode.summary'), default: 'commitid', options: ['commitid', 'tags', 'branch'] }),
    'delta-key': Flags.string({ char: 'k', summary: messages.getMessage('flags.delta-key.summary'), required: true }),
    'base-dir': Flags.directory({ char: 'd', summary: messages.getMessage('flags.base-dir.summary'), exists: true, default: 'force-app/main/default', deprecated: true}),
    'output-dir': Flags.directory({ char: 'p', summary: messages.getMessage('flags.output-dir.summary'), exists: true, default: 'manifest' }),
    granular: Flags.boolean({ char: 'g', summary: messages.getMessage('flags.granular.summary'), default: false}),
    'destructive-changes': Flags.boolean({ char: 'x', summary: messages.getMessage('flags.destructive-changes.summary'), default: false, hidden: true }),
  ...
  protected granularmode: boolean = false;
  protected packageDirectories: PackageDir[] = [];
  protected basedir: string = '';
  public async run(): Promise<DeltaResult> {
    const {flags} = await this.parse(Delta);
    ...
    this.basedir = fs.existsSync(flags['base-dir'])
    ? flags['base-dir']!
    : project.getDefaultPackage().fullPath;
    // flags
    let filter = 'AMRU';
  ...
  private processMetadataFileForPackage(file: any): void {
    ...
    if ((metadataType && fSuffix !== 'site' && fSuffix !== 'md') ?? (metadataType && fSuffix === 'md' && base.endsWith('-meta.xml'))){
        ...
        if (metadataType.strictDirectoryName){
            ...
            this.addMemberToPackage(tp,`${metadataType.folderType ? metadataDir[metadataTypIndex + 1] + '/' : ''}${fName}`);
        }
        ...
        const metadataTypIndex = metadataDir.findIndex(x => Object.values(registry.strictDirectoryNames).find(e => e === x) !== undefined);
        if (metadataTypIndex >= 0){
            metadataType = this.registryAccess.getTypeByName(Object.values(registry.strictDirectoryNames).find(e => e === metadataDir[metadataTypIndex])!);
            if(metadataType.name === 'CustomObject' && fSuffix !== 'object' && this.granularmode){
              const tp = this.initMetadataTypeInPackage(metadataType.children!.types[metadataType.children!.suffixes[fSuffix]].name);
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-redundant-type-constituents
  private getDeltaChanges(mode: 'branch' | 'tags' | 'commitid', deltakey: string, filter: string = 'AMRU'): string[] {
    const commandMap = {
