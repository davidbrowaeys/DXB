...
export type ApexTriggerCreateResult = {
  success: boolean;
};
...
export default class ApexTriggerCreate extends SfCommand<ApexTriggerCreateResult> {
  ...
  public async run(): Promise<ApexTriggerCreateResult> {
    const { flags } = await this.parse(ApexTriggerCreate);
    const project = await SfProject.resolve();
    const config: any = await project.resolveProjectConfig();
    const sobject = flags.sobject;
    const apiversion: number = flags['source-api-version'] ?? parseInt(config.sourceApiVersion, 10);

    const template = 'trigger';
    const vars =
      'className=' +
      sobject.replace('__c', '').replace('_', '') +
      'TriggerHandler,' +
      'triggerName=' +
      sobject.replace('__c', '').replace('_', '') +
      'Trigger,' +
      'apiVersion=' +
      apiversion.toPrecision(3) +
      ',' +
      'sobject=' +
      sobject;
