...
export default class ProfileBuild extends SfCommand<ProfileBuildResult> {
  ...
  public static readonly flags = {
    ...
    'source-dir': Flags.directory({ char: 'd', summary: messages.getMessage('flags.source-dir.summary'), default: 'force-app/main/default/profiles' })
  };
  ...
  public async run(): Promise<ProfileBuildResult> {
    ...
    if (profilename){
      ...
      return {
        result: fs.readdirSync(this.sourcepath).sort(generalSort)
  ...
  private buildProfile(profilename: string): string{
    ...
    if (fs.existsSync(profilepath+'/objectPermissions')) {
      fs.readdirSync(profilepath+'/objectPermissions').sort(generalSort).forEach(file => {
