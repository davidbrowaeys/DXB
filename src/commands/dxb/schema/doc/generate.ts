...
const getHelper = (executeFn: boolean, options: Handlebars.HelperOptions, it: any): string => executeFn ? options.fn(it) : options.inverse(it);
...
type SchemaDocGenResult = {
  success: boolean;
}
Messages.importMessagesDirectory(__dirname);
const messages = Messages.loadMessages('dxb', 'schema.doc.generate');

export default class SchemaDocGen extends SfCommand<SchemaDocGenResult> {
  ...
  public async run(): Promise<SchemaDocGenResult> {
    const {flags} = await this.parse(SchemaDocGen);
    ...
    if (!fs.existsSync(htmlPath))
      throw messages.createError('error.noHTMLTemplate', [htmlPath]);
    if (!fs.existsSync(cssPath))
    throw messages.createError('error.noStylesheetTemplate', [cssPath]);
    if (!fs.existsSync(pdfconfig))
    throw messages.createError('error.noPDFConfig', [pdfconfig]);
    ...
    standardObjects = await this.getObjectDefinition(
      standardObjects,
      'CustomObject',
      documentMeta.metadata.stdobjects
    );
    ...
    let flows: FlowDestruct[] = this.processFlow(
      (await this.getFlowDefinitions()).flat()
    );
    if (flows && this.packageCmps?.Flow) {
      flows = flows.filter((f: FlowDestruct) =>
        this.packageCmps?.Flow.includes(f.fullName!)
      );
    }
    ...
    if (
      ...
      apexClasses = apexClasses.filter((e: ApexClassDestruct) =>
    ...
    if (
      ...
      apexTriggers = apexTriggers.filter((e: ObjectDefinition) =>
    ...
    const connectedApps: any = await this.getConnectedAppUsage(
  ...
  private async getConnectedAppUsage(apps: ObjectDefinition[]): Promise<ConnectedAppDestruct[] | undefined> {
    ...
    const usagePromises = apps.map(async (app) => {
      try {
        const result: Record[] = (await this.connection.query(
          `SELECT COUNT() FROM OauthToken where AppName = '${app.Name}'`
        )).records;
        return { ...app, usage: result.length };
      } catch (err) {
        return { ...app, usage: 0 };
      }
    });
  ...
  private async getFlowDefinitions(): Promise<Flow[]> {
    ...
    if (flows) {
      ...
      return Promise.all(
        chunks.map(c => this.connection.metadata.read('Flow', c))
      );
    }
    return [];
  }
  ...
  private async getFlowDefinitionForSObject(sobjects: string[]): Promise<Flow[]> {
   ...
   */
  private getTriggerForSObject(
    sobjects: ObjectDefinition[],
    allTriggers: ObjectDefinition[]
  ): TriggerDescruct[] {
  ...
  private async getSharingRulesMetadata(fullNames: string[]): Promise<SharingRulesMetadata[][]> {
    ...
    return Promise.all(
      chunks.map(async (c) => 
        (this.toArray(await this.connection.metadata.read('SharingRules', c)) as SharingRules[])
        .map((sh: SharingRules) => {
        if (sh?.fullName) {
  ...
  private async getSSOSettingMetadata(apiVersion: string): Promise<SamlSsoConfig[]> {
    ...
    const list: FileProperties[] = this.toArray(
      await this.connection.metadata.list(types, apiVersion)
    );
    const fullNameList = list.map((e) => e.fullName);
    return this.toArray(
      await this.connection.metadata.read('SamlSsoConfig', fullNameList)
    ) as SamlSsoConfig[];
  }
   ...
   */
  private async getMetadataObject(type: string, fullNames: string[]): Promise<Array<MetadataDefinition<string,CustomObjectComponent>>> {
    const chunkSize = 5;
    const chunks: string[] = [];
    ...
    ) as Promise<Array<MetadataDefinition<string, CustomObjectComponent>>>;
  }
  /**
   ...
   */
  private async getObjectDefinition(sobjects: ObjectDefinition[], type: string, fullNames: string[]): Promise<ObjectDefinition[]> {
    ...
    const sharingRulesArray: SharingRulesMetadata[] = (
    ...
    .map((o: ObjectDefinition) => {
      ...
      const sharingRules = sharingRulesArray.find(
        (e) => e?.fullName && e.fullName === o.QualifiedApiName
      );
