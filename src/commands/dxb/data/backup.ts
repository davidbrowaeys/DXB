...
export default class DataBackup extends SfCommand<DataBackupResult> {
    ...
    public async run(): Promise<DataBackupResult> {
        ...
        if (mode === 'full'){   // run full mode using bulk api
            ...
            const lastDeltaTime: string = !config.delta.backupTime || config.delta.delta_type === 'since_full'
            ...
            outputdir = path.join(
                outputdir,'cycle-' +config.full.cycle,'delta', config.delta.delta_type === 'since_full'
                    ? 1
                    : config.delta.cycle.toString()
            );
    ...
    private async generateQuery(connection: Connection, objectname: string, allfields: boolean): Promise<string> {
        const soql = ['SELECT'];
        let fields = [];
        if (allfields){
    ...
    private queryAll(connection: Connection, soql: string, outputFile: string): Promise<string>{
     ...
     */
    // eslint-disable-next-line class-methods-use-this
    private async getObjectFields(connection: Connection, objectname: string): Promise<string[]> {
        const sobject = connection.sobject(objectname);
        const meta: DescribeSObjectResult = await sobject.describe();
