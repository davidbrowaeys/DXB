...
type PackageDependencyInstallResult = {
  success: boolean;
}
...
export default class PackageDependencyInstall extends SfCommand<PackageDependencyInstallResult> {
  ...
  public async run(): Promise<PackageDependencyInstallResult> {
    const {flags} = await this.parse(PackageDependencyInstall);
      ...
      if ( pkg.dependencies){
        pkg.dependencies.forEach( elem => {
          try{
            ...
            this.log(`sf package install --package "${packageID}" --target-org ${this.orgName} --wait 600 --json --no-prompt`);
            // console.log('Installing',packageID,installedPackage.find(p => packageID === ''));
            const output = JSON.parse(exec(`sf package install --package "${packageID}" --target-org ${this.orgName} --wait 600 --json --no-prompt`).toString());
