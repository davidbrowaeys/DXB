...
export type PermissionsetCleanResult = {
  success: boolean;
};

export default class PermissionsetClean extends SfCommand<PermissionsetCleanResult> {
  ...
  public async run(): Promise<PermissionsetCleanResult> {
    const { flags } = await this.parse(PermissionsetClean);
  ...
  private filterFieldPermissions(result: any): void {
    ...
    } else if (fieldPermissions.readable === 'false' && fieldPermissions.editable === 'false') {
      delete result.PermissionSet.fieldPermissions;
    }
  }
  ...
  private filterUserPermissions(result: any): void {
    ...
    if (Array.isArray(result.PermissionSet.userPermissions)) {
      result.PermissionSet.userPermissions = this.filterArrayByAttributeValue(userPermissions, 'enabled', 'true');
