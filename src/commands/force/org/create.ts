...
export default class Org extends SfdxCommand {
  ...
  public static description = 'Creaste deloitte scratch org';

  public static examples = [
  `$ deloitte force:org:create --includepackages --defaultorg --orgname myorg`,
  `$ deloitte force:org:create --durationday 10 --defaultorg --orgname myorg`
  ];
  ...
  public async run() {
    ...
    if (!config.plugins || !config.plugins.deloitteforce){
      throw new SfdxError('Plugin definition deloitteforce is missing in sfdx-project.json, make sure to setup plugin.');
    }
    config = config.plugins.deloitteforce;
    ...
    this.ux.log('\x1b[91m%s\x1b[0m', `Welcome to DeloitteForce CLI! We are now creating your scratch org[${orgname}]...`);

    var output = await this.create_scratch_org(orgname, defaultorg, durationdays);
    console.log(output);

    //UPDATE WORKFLOWS
    console.log(exec(`deloitte force:org:setdefault -u ${orgname}`).toString());
