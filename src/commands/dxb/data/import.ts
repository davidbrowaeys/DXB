...
export default class DataImport extends SfCommand<DataImportResult> {
  ...
  public static readonly aliases = ['dxb:data:transfer:import'];
  ...
  private async import(config: GenericObject): Promise<void> {
    ...
    const importfile = path.join(this.importdir ?? '', `tmp_${config.filename as string}`);
    this.log(
      messages.getMessage('log.registerImport', [`[\x1b[33m${config.objectName as string},${filepath}\x1b[0m]`])
    );
  ...
  private initFile(config: GenericObject): GenericObject {
    ...
    config.fields.forEach((key: string) => {
      ...
      if (key === 'recordtype.developername') {
        // replace RecordType.DeveloperName column by recordtypeid
  ...
  private async upsertRecords(objectName: string, filename: string, externalIdField = 'Id'): Promise<BatchResult> {
    try {
      ...
      const rets =
        (await this.connection?.bulk.load(objectName, 'upsert', { extIdField: externalIdField }, csvFileIn)) ?? [];
      ...
      this.log(
        messages.getMessage('log.loadResult', [
          `\x1b[32m${results.success.length}\x1b[0m`,
          `\x1b[31m${results.errors.length}\x1b[0m`,
        ])
      );
      fs.unlinkSync(filename);
      return results;
