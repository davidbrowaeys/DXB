...
export type ApexCoverageCheckResult = {
  success: boolean;
};
type Coverage = {
  name: string;
...
export default class ApexCoverageCheck extends SfCommand<ApexCoverageCheckResult> {
  ...
  public async run(): Promise<ApexCoverageCheckResult> {
    const { flags } = await this.parse(ApexCoverageCheck);
