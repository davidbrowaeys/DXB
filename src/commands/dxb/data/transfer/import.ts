...
const messages = Messages.loadMessages('dxb', 'data.transfer.import');

export default class DataTransferImport extends SfCommand<DataTransferImportResult> {
  ...
  public async run(): Promise<DataTransferImportResult> {
    
    const {flags} = await this.parse(DataTransferImport);
