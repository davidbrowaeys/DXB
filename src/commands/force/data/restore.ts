...
export default class DataRestore extends SfdxCommand {
    ...
    public static examples = [
        `$ deloitte force:data:backup -f backup-def.json -d backup -m full -u devhub`,
        `$ deloitte force:data:backup -f backup-def.json -d backup -m delta -u devhub`
    ];
    ...
    private async restoreFull(){
        ...
        await files.forEach( async (f:string) => {
            if (f.endsWith('.csv')){
                const res = await this.processFile(path.join(this.backupdir,'full'),f);
            }
        });
    }
    ...
    private async cleanupNewRecords(){
        var fromDate = this.setting.delta.backupTime;
        //for each object run query
        this.setting.full.objects.forEach( async (object) => {
            this.ux.startSpinner(`Processing [${object}]`);
            var results = await this.deleteRecords(object,fromDate);
