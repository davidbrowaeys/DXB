...
export type GraphqlConvertResult = {
  graphqlQuery: string;
};
  ...
  public convertSOQLToGraphQL(soqlQuery: string): string {
    ...
    const graphqlFields = fields?.map((field) => {
      if (field.toLocaleLowerCase() === 'id') {
        ...
        return nestedFields.reduceRight((acc, curr) => `${curr} { ${acc} }`, 'value');
      } else {
        return `${field} { value }`;
      }
    });
    ...
    */
    let objectParams = structuredQuery.limit ? `first:${structuredQuery.limit as number}` : '';
    ...
    return `${objectName as string}${objectParams}{
      edges {
        node{
          ${graphqlFields?.join('\n')}
        }
