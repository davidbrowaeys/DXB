...
export default class DeltaGenerate extends SfdxCommand {
  ...
  public async run() {
    ...
    if (mode === 'branch'){
      ...
      if (prevTag) {
        files = exec(`git diff $(git describe --match ${prevTag}* --abbrev=0)..HEAD --name-only | grep force-app | sort | uniq`).toString().split('\n');
      }else{
        files = exec(`git diff $(git describe --tags --abbrev=0)..HEAD --name-only | grep force-app | sort | uniq`).toString().split('\n');
      }
    }else{
