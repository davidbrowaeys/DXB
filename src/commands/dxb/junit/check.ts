...
type JUnitReviewResult = {
  success: boolean;
}
Messages.importMessagesDirectory(__dirname);
const messages = Messages.loadMessages('dxb', 'junit.check');
export default class JUnitReview extends SfCommand<JUnitReviewResult> {

  public static readonly summary = messages.getMessage('summary');

  public static readonly examples = [
    '$ sfdx dxb:junit:check '
  ];

  public static readonly flags = {
    'junit-path': Flags.file({ char: 'p', summary: messages.getMessage('flags.junit-path.summary'), required:true, exists: true }),
    'time-threshold': Flags.integer({ char: 't', summary: messages.getMessage('flags.time-threshold.summary'), default: 1}),
    'flag-as-error': Flags.boolean({ char: 'e', summary: messages.getMessage('flags.flag-as-error.summary'), default: false})
  };

  public async run(): Promise<JUnitReviewResult> {
    const {flags} = await this.parse(JUnitReview);
