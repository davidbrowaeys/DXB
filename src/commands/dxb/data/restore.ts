...
export default class DataRestore extends SfCommand<DataRestoreResult> {
  ...
  public static readonly flags = {
    ...
    'backup-dir': Flags.directory({char:'f',summary: messages.getMessage('flags.backup-dir.summary'),required:true, exists: true})
  };
  ...
  private async upsertRecords(object: string, filename: string, externalIdField = 'Id'): Promise<BulkIngestBatchResult> {
  ...
  private async getSupportedObjectFields(objectname: string): Promise<string[]>{
    const meta: DescribeSObjectResult = await this.connection!.sobject(objectname).describe();
