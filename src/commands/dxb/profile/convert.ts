...
export default class ProfileConvert extends SfdxCommand {
    ...
    public async run() {
        ...
        if (profilename) {
            try {
                this.convertProfile(profilename, sourcepath);
            } catch (err) {
     ...
     */
    public convertProfile(profilename: string, sourcepath: string) {
            ...
            if (err){
                ...
                if (result.Profile.layoutAssignments) {
                    ...
                    result.Profile.layoutAssignments.forEach(function (elem) {
                        const key = elem.recordType
                        ? `${elem.layout.split('-')[0]}-${elem.recordType}`
                        : elem.layout;
