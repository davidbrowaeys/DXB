...
export type DataBulkQueryResult = {
  outputFile: string;
};

export default class DataBulkQuery extends SfCommand<DataBulkQueryResult> {
  ...
  public async run(): Promise<DataBulkQueryResult> {
    const { flags } = await this.parse(DataBulkQuery);
    ...
    if (!this.connection?.accessToken || !this.connection.instanceUrl) {
      throw new SfError(
        messages.getMessage('error.message.noConfiguration'),
        messages.getMessage('error.name.invalidConnection')
      );
    }
    // handle query
    if (!this.query && !this.objectname) {
      // invalid arguments
      throw new SfError(
        messages.getMessage('error.message.queryOrObject'),
        messages.getMessage('error.name.invalidFlags')
      );
    } else if (this.query) {
      this.objectname = this.query
        .toUpperCase()
        .replace(/\([\s\S]+\)/g, '')
        .match(/FROM\s+(\w+)/i)![1];
      if (!this.objectname) {
        throw new SfError(
          messages.getMessage('error.message.invalidSOQL'),
          messages.getMessage('error.name.invalidSOQL')
        );
      }
      const fieldSelector = this.query
        .replace(/\([\s\S]+\)/g, '')
    ...
    const filename = flags['file-name'] ?? this.objectname + '.csv';
