...
export default class ApiAlign extends SfCommand<ApiAlignResult> {
  ...
  public async run(): Promise<ApiAlignResult> {
    ...
    const directories: string[] =
      flags.directory ?? projectConfig.packageDirectories.map((packageDir: PackageDir) => packageDir.path);
    const metadataTypes: string[] | undefined = flags['metadata-type'];
    this.log(metadataTypes?.toString());
    ...
    directories.forEach((rootPath: string) => {
      const filesWithApi: string[] = !fs.lstatSync(rootPath).isDirectory()
        ? [rootPath]
        : (this.findFilesWithTag(rootPath, 'apiVersion') as string[]).filter(
            (f: string) => !apiAlignmentExclusion.includes(f)
          );
      ...
      filesWithApi.forEach((f: string) => {
        void this.processFile(f, projectApi, metadataTypes);
      });
    });
    return { success: true };
  }
   ...
   */
  public findFilesWithTag(filePath: string, tag: string): string | Array<string | undefined> {
    if (fs.lstatSync(filePath).isDirectory()) {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      return fs
        .readdirSync(filePath)
        .flatMap((entry: string) => this.findFilesWithTag(`${filePath}/${entry}`, tag))
        .filter((e) => e !== '');
   ...
   */
  private async processFile(filePath: string, projectApi: string, metadataTypes?: string[]): Promise<void> {
    const fileContent: string = fs.readFileSync(filePath, { encoding: 'utf-8' });
    const parser = new xml2js.Parser({ explicitArray: false });
    const result: any = await parser.parseStringPromise(fileContent);

    const root = Object.keys(result)[0];
    this.log(root);
    if (metadataTypes?.includes(root) === false) {
      // in case specific metadata types are wanted, skip if this type is not one of them
      return;
    }
    delete result[root]['$'];
    result[root]['@'] = {
    ...
    this.log(`Change API Version of ${filePath} from ${result[root].apiVersion as string} to ${projectApi}`);
    result[root].apiVersion = projectApi;
    const xml = js2xmlparser.parse(root, result[root], {
